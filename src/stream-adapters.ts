...
export class WritableAdapter extends Writable {
  ...
  constructor(webStream: FileSystemWritableFileStream, options?: WritableOptions) {
    super(options);
    this._webStream = webStream;
  }

  _write(chunk: any, enc: string, cb: (error?: Error | null) => void) {
    this._webStream.write(chunk as FileSystemWriteChunkType)
      .then(() => cb())
      .catch((reason) => cb(reason));
  }

  _destroy(error: Error | null, cb: (error: Error | null) => void): void {
    this._webStream.close()
      .then(() => cb(null))
      .catch((reason) => cb(reason));
  }
}